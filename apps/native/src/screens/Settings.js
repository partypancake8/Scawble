...
function Seg({ options, value, theme, onSelect }) {
  return (
      ...
      {options.map(([v, l]) => {
        ...
        return (
          <Pressable key={v} onPress={() => onSelect(v)}
            style={[styles.segbtn, { backgroundColor: on ? theme.ink : 'transparent', borderColor: on ? 'transparent' : theme.line }]}>
            <Text style={{ fontFamily: FONT_SEMI, fontSize: 12, color: on ? theme.paper : theme.muted }}>{l}</Text>
          </Pressable>
        );
      })}
    </View>
...
export default function Settings({ settings, difficulty, theme, onToggle, onChange, onSetDifficulty, onBack }) {
  const Row = ({ label, children }) => (
    <View style={[styles.setrow, { backgroundColor: theme.surface, borderColor: theme.line }]}>
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.paper }]}>
      ...
      <View style={styles.list}>
        ...
        <Row label="Difficulty"><Seg options={[['casual', 'Casual'], ['skilled', 'Skilled'], ['expert', 'Expert'], ['brutal', 'Brutal']]} value={difficulty} theme={theme} onSelect={onSetDifficulty} /></Row>
      </View>
